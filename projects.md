...
Developed a full-stack web application during a 24-hour hackathon to help users navigate social welfare programs such as SNAP, Medicaid, and HEAP. The system simplifies complex eligibility requirements into an accessible interface, allowing users to quickly understand what support they may qualify for.

### What I did

- **Eligibility matching system**
  - Implemented a rule-based decision engine in Python that evaluates user inputs (household size, income, employment status, etc.) against structured eligibility criteria for SNAP, Medicaid, and HEAP
  - Translated policy guidelines into conditional logic and threshold-based rules, enabling deterministic and consistent eligibility evaluation

- **Contextual question answering (RAG)**
  - Built a lightweight retrieval-augmented generation (RAG) pipeline that retrieves relevant program rules and documentation based on user queries
  - Combined retrieved policy context with user-specific inputs to generate grounded, context-aware responses beyond simple yes/no outputs
  - Structured program data for retrieval to ensure responses remain aligned with official eligibility guidelines

- **Location-based assistance**
  - Integrated geolocation-based lookup to surface nearby assistance offices using user location input
  - Structured location data for efficient querying and display within the frontend interface

- **Full-stack implementation**
  - Developed a React-based frontend for form handling, state management, and results visualization
  - Built a Python backend (API layer) to process user inputs, execute eligibility logic, and return structured responses
  - Designed clean data flow between frontend and backend using JSON-based request/response patterns

- **System design under constraints**
  - Architected the application with modular components (eligibility logic, retrieval pipeline, UI) within a 24-hour development cycle
  - Prioritized clarity, low latency, and usability for real-time interaction

### Why it matters

This project focuses on applying technical systems to real-world accessibility challenges:

- Translating policy rules into structured logic
- Connecting user context with program requirements
- Designing systems that reduce friction in accessing essential resources

It reflects an approach to building tools that are not only functional, but socially meaningful.

### Link

[GitHub repository](https://github.com/alicepark0703/benefits-navigator)
